...
export default function VcClPage() {
  ...
  // Shared qty+form block, inlined twice
  const qtyAndForm = (
    <>
      {/* QTY selector */}
      <div className="flex flex-col gap-3 mb-4">
        <button type="button" onClick={() => setQty(1)} className="relative w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all shadow-sm active:scale-95"
          style={{ border: qty === 1 ? `2px solid ${GOLD}` : "2px solid #e5e7eb", background: qty === 1 ? "#fdf8ee" : "#fff" }}>
          <div className="shrink-0 w-5 h-5 rounded-full flex items-center justify-center" style={{ border: qty === 1 ? `2px solid ${GOLD}` : "2px solid #d1d5db" }}>
            {qty === 1 && <div className="w-2.5 h-2.5 rounded-full" style={{ background: GOLD }} />}
          </div>
        ...
        <button type="button" onClick={() => setQty(2)} className="relative w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all shadow-sm active:scale-95"
          style={{ border: qty === 2 ? `2px solid ${GOLD}` : "2px solid #e5e7eb", background: qty === 2 ? "#fdf8ee" : "#fff" }}>
          <span className="absolute -top-3 right-3 text-black text-xs font-black px-2 py-1 rounded" style={{ background: GOLD }}>وفر {SAVING} درهم</span>
          <div className="shrink-0 w-5 h-5 rounded-full flex items-center justify-center" style={{ border: qty === 2 ? `2px solid ${GOLD}` : "2px solid #d1d5db" }}>
            {qty === 2 && <div className="w-2.5 h-2.5 rounded-full" style={{ background: GOLD }} />}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input type="text" value={form.name} onChange={(e) => { const f = { ...form, name: e.target.value }; setForm(f); saveDraft(f, qty, price) }}
                placeholder="الاسم الكامل" className={inputCls} onFocus={onFocusGold} onBlur={onBlurGray} />
            </div>
            <div>
              ...
              <input type="text" value={form.city} onChange={(e) => { const f = { ...form, city: e.target.value }; setForm(f); saveDraft(f, qty, price) }}
                placeholder="مثلاً: كازا، الرباط..." className={inputCls} onFocus={onFocusGold} onBlur={onBlurGray} />
            </div>
            <div>
  ...
  return (
    <div className="min-h-screen text-gray-900" dir="rtl" style={{ backgroundColor: "#fff", fontFamily: "var(--font-cairo), Cairo, sans-serif" }}>
      ...
      <img src="/products/WrtqtMIlBcTBN0NXB9q44lPH6kk0elO7pmA47evz.webp" alt="أسبيراتور 6 في 1 للسيارة"
      ...
      <div className="px-4 pt-5 pb-3 max-w-lg mx-auto">
        <h1 className="font-black text-xl text-gray-900 mb-2 text-right">أسبيراتور و سوفلور 6 في 1 — للسيارة والدار</h1>
        ...
        <p className="text-red-500 font-black text-sm mt-0.5">⏳ العرض سينتهي خلال ساعات قليلة</p>
      </div>
      ...
      {/* ══ GIFs + FEATURES ══ */}
      <div className="px-4 py-6 bg-white max-w-lg mx-auto text-center">
        <p className="font-black text-xl text-gray-900 mb-4">شفط و امتصاص الأوساخ</p>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/products/JpCQxc8s29Q6GGnEv8h6O0EQW0UKFWDeflYQn9sh.gif" alt="شفط" className="w-full h-auto rounded-xl mb-6" loading="lazy" />
        <p className="font-black text-xl text-gray-900 mb-4">النفخ</p>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/products/0QiqKF3M5kr6BMSvuORIqZxhPvyw600Ej7rbm3iL.gif" alt="نفخ" className="w-full h-auto rounded-xl mb-6" loading="lazy" />
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/products/QStj65FC8vb0NDoELLfJPlzOuGObMGg4hgBWOrnW.webp" alt="مميزات" className="w-full h-auto rounded-xl" loading="lazy" />
      </div>
